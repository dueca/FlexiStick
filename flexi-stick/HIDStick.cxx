...
SDL_JoystickID HIDStick::getJoystickID() { return -1; }
...
void HIDStick::propagate()
{
  ...
  unsigned idx = 0;
#endif
  for (hats_t::iterator ih = hats.begin(); ih != hats.end(); ih++) {
#ifdef PDEBUG
    if ((*ih)->changed) {
      PDEB("Propel hat " << idx);
    }
  ...
  idx = 0;
#endif
  for (axes_t::iterator ia = axes.begin(); ia != axes.end(); ia++) {
#ifdef PDEBUG
    if ((*ia)->changed) {
  ...
  idx = 0;
#endif
  for (buttons_t::iterator ib = buttons.begin(); ib != buttons.end(); ib++) {
#ifdef PDEBUG
    if ((*ib)->changed) {
...
HIDStick::HatFilter::HatFilter(const char hatdir) :
  value(0xff)
{
  switch (hatdir) {
  case 'u':
  ...
  case 'l':
    ...
    break;
  case 'r':
    filter = SDL_HAT_RIGHT;
    break;
  }
}

void HIDStick::HatFilter::propagate(int &v, unsigned idx)
{
